...
def get_top_match(reference_image_path, folder):
    ...
    for img in image_files:
        ...
        result = DeepFace.verify(reference_image_path, compare_path, model_name='Facenet', enforce_detection=False)
        distance = result.get('distance', float('inf'))
    ...
    return {'img': os.path.splitext(best_match)[0].replace('-', ' '), 'distance': best_distance, 'image_base64': encoded_string}
